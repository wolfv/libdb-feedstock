...
Once the `conda-forge` channel has been enabled, `libdb` can be installed with `conda`:
...
It is possible to list all of the versions of `libdb` available on your platform with `conda`:
...
```
mamba search libdb --channel conda-forge
```
...
mamba repoquery search libdb --channel conda-forge

# List packages depending on `libdb`:
mamba repoquery whoneeds libdb --channel conda-forge

# List dependencies of `libdb`:
mamba repoquery depends libdb --channel conda-forge
```
